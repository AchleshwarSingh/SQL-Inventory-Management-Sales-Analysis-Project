...
CREATE TABLE PRODUCT (
    ...
    CATEGORY CHAR(2) NOT NULL CHECK (CATEGORY IN ('IT', 'HA', 'HC')),
    SID CHAR(5) NOT NULL,
...
CREATE TABLE CUSTOMER (
    ...
    CCITY VARCHAR(20),
    CPHONE CHAR(10) NOT NULL UNIQUE,
    CEMAIL VARCHAR(40),
    DOB DATE NOT NULL
);
...
SELECT SUM(OPPRICE) FROM ORDER_DETAILS ; 
...
SELECT P.PID , P.PDESC , SUM(OQTY*PRICE) AS TOTAL_REVENUE 
...
SELECT P.SID , SUM(OQTY*PRICE) AS TOTAL_REVENUE 
	FROM ORDER_DETAILS O 
    INNER JOIN PRODUCT P
    ON P.PID = O.PID
    GROUP BY P.SID
...
WITH SUPPLIER_REVENUES AS (
	SELECT S.SID , S.SNAME , SUM(O.OQTY*P.PRICE) AS SUPPLIER_REVENUE 
    FROM ORDER_DETAILS O 
    INNER JOIN PRODUCT P ON P.PID=O.PID
	INNER JOIN SUPPLIER S ON P.SID=S.SID
    GROUP BY S.SID , S.SNAME ) , 
    TOTAL_REVENUE AS (
	SELECT SUM(SUPPLIER_REVENUE) AS GRAND_TOTAL FROM SUPPLIER_REVENUES ) 
	
    SELECT 
		SR.SID ,
        SR.SNAME , 
        SR.SUPPLIER_REVENUE , 
        ROUND((SR.SUPPLIER_REVENUE * 100 ) / TR.GRAND_TOTAL, 2) AS REVENUE_CONTRIBUTION_PERCENT
	FROM 
		SUPPLIER_REVENUES SR
